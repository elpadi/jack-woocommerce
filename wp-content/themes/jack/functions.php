...
{
    printf(
        '<p class="store-headline">%s</p>',
        "Jack  -  the largest publication in the world.<br>
        1000 limited edition / issue<br>
        signed + numbered"
    );
}
...
if (is_admin() == false) {
    ...
    add_filter('get_custom_logo', function ($html) {
        ...
        return str_replace(
            ...
            sprintf(
                '<img src="%s" alt="%s">',
                get_stylesheet_directory_uri() . '/assets/icons/logo.svg',
                get_bloginfo('name')
            ) . $end,
            $html
        );
    ...
    add_filter('woocommerce_dropdown_variation_attribute_options_html', function ($html, $args) {
        ...
        $html .= implode('', array_map(function ($v) {
            return $v->get_image();
        }, $variations));
    ...
    add_action('wp_head', function () {
        add_action('woocommerce_before_add_to_cart_form', function () {
            ...
            if ($slug = get_post_meta($post->ID, 'website_slug', true)) {
                printf(
                    '<p><a href="https://thejackmag.com/issues/%s/layouts" target="_blank" rel="nofollow">View Issue Content</a></p>',
                    $slug
                );
            }
        });
        ...
        if (is_singular('product')) {
            add_action('woocommerce_before_main_content', 'print_store_headline', 30);
        }
    });
}
